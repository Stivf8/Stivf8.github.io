        ...
        fetch(`http://apiprosom.somee.com/api/vendedor`)
            ...
            .then((data) => {
                ...
                data.forEach((user) => {
                    ...
                    row.innerHTML = `
                        <td class="text-center">${user.IdV}</td>
                        ...
                        <td> <button id="editar"  value=${user.IdV} class="btn btn-warning" >editar</button> </td>
                        <td> <button id="borrar"  value=${user.IdV} class="btn btn-danger" >eliminar</button> </td>
                    `;
...
if (event.target.id == "borrar") {
  ...
  if (confirmacion == true) {
    fetch(`http://apiprosom.somee.com/api/vendedor/${event.target.value}`, {
      method: "DELETE",
    })
      .then((response) => {
        if (!response.ok) {
  ...
  window.location.href = "../view/editarVendedor.html?id=" + event.target.value; // Agrega el parámetro a la URL
}
